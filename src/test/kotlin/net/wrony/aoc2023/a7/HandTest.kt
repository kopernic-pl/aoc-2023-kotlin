...
class HandTest {
    ...
    fun `hand of five aces is greater than HighCard`() {
        val hand1 = Hand.fromTextLine("AAAAA 1", valMapper)
        val hand2 = Hand.fromTextLine("23456 2", valMapper)
    ...
    fun `hand of five aces is greater than hand of five kings`() {
        val hand1 = Hand.fromTextLine("AAAAA 1", valMapper)
        val hand2 = Hand.fromTextLine("KKKKK 2", valMapper)
        assertTrue(hand1 > hand2)
    }

    @Test
    fun `HighCard starting with ace is greater than HighCard starting with king`() {
        val hand1 = Hand.fromTextLine("A2345 1", valMapper)
        val hand2 = Hand.fromTextLine("K5678 2", valMapper)
        assertTrue(hand1 > hand2)
    }

    @Test
    fun `HighCard starting with king is smaller than HighCard starting with king`() {
        val hand1 = Hand.fromTextLine("K5678 2", valMapper)
        val hand2 = Hand.fromTextLine("A2345 1", valMapper)
